...
import isAllowed from "../api/users/getallowed";
...
const Painel = () => {
  ...
  // UPDATE
  async function setInput(valor: any, id: any) {
  ...
  const allowedRequest = async () => {
    ...
    if (sessao.data) {
      ...
      const permitted = await api.post("users", {
        //email: "victor@lunnar.team",
        email: sessao.data.user.email,
      });
      console.log("permitted.data");
      console.log(permitted.data);
      setPermitted(permitted.data);
      return permitted;
    }
  };
  useEffect(() => {
  ...
  if (permitted) {
    ...
    return <h1>USUARIO NAO PERMITIDO</h1>;
  }
};
